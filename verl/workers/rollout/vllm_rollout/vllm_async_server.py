...
class vLLMHttpServerBase:
    ...
    async def generate(
        ...
        ##################
        original_logprobs = sampling_params.pop("logprobs", False)
        if isinstance(original_logprobs, int) and original_logprobs > 0:
            # 如果 logprobs 本身就是整数 N，直接使用
            sampling_params["logprobs"] = original_logprobs
        elif original_logprobs: 
            # 如果 logprobs=True，回退到默认行为 (设为 0)
        ...
        if sampling_params.logprobs is not None:
            top_logprobs_data = final_res.outputs[0].logprobs

        return TokenOutput(
            token_ids=token_ids, log_probs=log_probs, routed_experts=routed_experts, stop_reason=stop_reason, top_logprobs=top_logprobs_data,
        )
